...
const StoreApp: React.FC = () => {
  ...
  // Logic Constants
  const ORDER_START_HOUR = 5;
  const ORDER_END_HOUR = 23; // Extended for demo
  ...
  const checkTimeRestriction = () => {
    ...
    if (hour < ORDER_START_HOUR || hour >= ORDER_END_HOUR) {
      alert(`当前不在下单时间内 (${ORDER_START_HOUR}:00 - ${ORDER_END_HOUR}:00)`);
      return false;
    }
    return true;
  };
  ...
  const updateCart = (product: Product, delta: number) => {
    ...
    setCart(prev => {
      ...
      if (delta > 0 && newQty > product.maxOrder) {
          // alert(`超过该商品每日限购数量 (${product.maxOrder}${product.unit})`);
          return prev;
      }
  ...
  if (!isLoggedIn) {
    return (
      <div className="flex flex-col h-full bg-white relative overflow-hidden font-sans selection:bg-green-100">
        <div className="flex-1 flex flex-col items-center justify-center p-8 z-10 animate-in fade-in duration-700">
           <div className="w-24 h-24 bg-gradient-to-tr from-[#00B42A] to-[#4CD964] rounded-[2rem] flex items-center justify-center mb-8 shadow-2xl shadow-green-500/20 ring-4 ring-green-50">
              <ShoppingBag size={40} className="text-white drop-shadow-sm" strokeWidth={2} />
           </div>
           ...
           {!isBinding ? (
             <div className="w-full space-y-4">
                <div className="space-y-3 mb-8">
                  <input type="text" placeholder="账号 / 手机号" className="w-full bg-gray-50 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-[#00B42A]/20 transition-all outline-none" />
                  <input type="password" placeholder="密码" className="w-full bg-gray-50 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-[#00B42A]/20 transition-all outline-none" />
                </div>
                
                <button 
                  onClick={() => setIsLoggedIn(true)}
                  className="w-full bg-[#00B42A] text-white py-4 rounded-2xl font-bold text-base shadow-lg shadow-green-500/20 active:scale-[0.98] transition-all hover:bg-[#009C25]"
                >
                  登录
                </button>
                <div className="text-center">
                  <button 
                    ...
                    className="text-[#00B42A] text-sm font-medium hover:underline"
                  >
                    扫码绑定新设备
                  </button>
                </div>
  ...
  const renderHome = () => (
    <div className="flex flex-col h-full bg-[#F7F8FA] font-sans">
      {/* Sticky Header */}
      <div className="px-6 pt-14 pb-4 bg-white/80 backdrop-blur-xl sticky top-0 z-30 transition-all">
        <div className="flex justify-between items-center mb-5">
           <div>
             ...
             <div className="flex items-center gap-1.5 text-gray-400 text-xs mt-2">
               <MapPin size={12} strokeWidth={2.5} /> <span className="truncate max-w-[200px] font-medium">五华区翠湖路88号</span>
             </div>
           </div>
        ...
        <div className="bg-gray-50 rounded-2xl flex items-center px-4 py-3.5 transition-all hover:bg-white hover:shadow-sm focus-within:bg-white focus-within:ring-2 focus-within:ring-[#00B42A]/10 focus-within:shadow-md">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-28 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        ...
        <div className="mt-4 bg-white rounded-[32px] p-1 relative shadow-[0_10px_30px_rgba(0,180,42,0.06)] overflow-hidden group">
           <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-green-50 via-transparent to-transparent opacity-50 pointer-events-none rounded-tr-[32px]"></div>
           
           <div className="bg-gradient-to-br from-[#F9FAFB] to-white rounded-[28px] p-6 border border-white/50 relative z-10">
               <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="font-bold text-gray-900 text-lg">今日订货</h3>
                    <div className="flex items-center gap-1.5 text-xs text-gray-400 font-medium mt-1">
                        <Clock size={12} /> 6月 24日
                    </div>
                  </div>
                  <div className="bg-green-50 text-[#00B42A] px-3 py-1 rounded-full text-xs font-bold">营业中</div>
               </div>
               
               <div className="grid grid-cols-3 gap-2 mb-8">
                  {[
                      { label: '未下单', count: 1, color: 'text-gray-300', bg: 'bg-gray-50' },
                      { label: '审核中', count: 3, color: 'text-blue-500', bg: 'bg-blue-50' },
                      { label: '已审核', count: 12, color: 'text-[#00B42A]', bg: 'bg-green-50' }
                  ].map((item, i) => (
                      <div key={i} className={`flex flex-col items-center justify-center p-3 rounded-2xl ${item.bg}`}>
                         <span className={`text-xl font-bold mb-0.5 ${item.color}`}>{item.count}</span>
               ...
               <button 
                 ...
                 className="w-full bg-[#00B42A] text-white py-4 rounded-2xl font-bold text-base shadow-lg shadow-green-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group-hover:bg-[#009C25]"
               >
        ...
        <div className="mt-10">
           <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-gray-400 text-xs font-medium active:text-[#00B42A] transition-colors cursor-pointer" onClick={() => setActiveTab('shop')}>全部分类</span>
           </div>
           <div className="grid grid-cols-4 gap-3">
              {[
                  { name: '水果', icon: '🍇', color: 'bg-purple-50 text-purple-600' },
                  { name: '蔬菜', icon: '🥦', color: 'bg-green-50 text-green-600' },
                  { name: '乳制品', icon: '🥛', color: 'bg-blue-50 text-blue-600' },
                  { name: '肉类', icon: '🥩', color: 'bg-red-50 text-red-600' }
              ].map((cat, i) => (
                 <div key={cat.name} className="flex flex-col items-center gap-2 cursor-pointer group" onClick={() => { setSelectedCategory(i===1?'叶菜类':'全部'); setActiveTab('shop'); }}>
                    <div className={`w-[72px] h-[72px] rounded-[24px] flex items-center justify-center text-2xl transition-all duration-300 group-active:scale-90 shadow-sm border border-transparent group-hover:border-gray-100 group-hover:shadow-md ${cat.color} bg-opacity-60`}>
                        {cat.icon}
                    </div>
        ...
        <div className="mt-10 pb-6">
           <h3 className="font-bold text-gray-900 text-lg mb-5">热销推荐</h3>
           <div className="grid grid-cols-2 gap-4">
              {MOCK_PRODUCTS.slice(0, 2).map((p, i) => (
                 <div key={p.id} className="bg-white p-3 rounded-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] active:scale-[0.98] transition-all cursor-pointer hover:shadow-[0_8px_25px_rgba(0,0,0,0.06)] group" onClick={() => updateCart(p, 1)}>
                    <div className="aspect-square bg-gray-50 rounded-[18px] mb-3 overflow-hidden relative">
                       <img src={p.imageUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
                       {i===0 && (
                           <div className="absolute top-2 left-2 bg-black/80 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-lg">
                             TOP 1
                           </div>
                       )}
                       <div className="absolute bottom-2 right-2 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-[#00B42A] opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 shadow-sm">
                    ...
                    <div className="px-1.5 pb-1">
                        ...
                        <div className="flex justify-between items-end">
                            <span className="text-gray-900 font-bold text-lg"><span className="text-xs font-normal text-gray-400 mr-0.5">¥</span>{p.price}</span>
                        </div>
                    </div>
                 </div>
              ))}
  ...
  const renderShop = () => (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="pt-14 pb-4 px-4 bg-white/95 backdrop-blur-xl flex items-center justify-between z-20 sticky top-0 border-b border-gray-50">
         <button onClick={() => setActiveTab('home')} className="p-2 -ml-2 text-gray-800 active:text-gray-400 transition-colors"><ChevronLeft size={24} /></button>
         <h1 className="text-base font-bold">商品下单</h1>
         <div className="p-2 -mr-2 text-gray-800"><Search size={22} strokeWidth={2} /></div>
      </div>

      <div className="flex-1 flex overflow-hidden relative">
         {/* Sidebar - Floating Indicator */}
         <div className="w-[92px] bg-[#F7F8FA] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] py-2 space-y-1">
            {categories.map(cat => (
               <div 
                 key={cat} 
                 onClick={() => setSelectedCategory(cat)}
                 className={`h-[72px] flex flex-col items-center justify-center gap-1.5 relative transition-all cursor-pointer mx-2 rounded-2xl ${selectedCategory === cat ? 'bg-white shadow-sm' : 'hover:bg-gray-100/50 text-gray-400'}`}
               >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${selectedCategory === cat ? 'bg-[#00B42A] text-white scale-110 shadow-lg shadow-green-500/30' : 'bg-transparent'}`}>
                     {selectedCategory === cat ? <ShoppingBag size={14} /> : <div className="w-1.5 h-1.5 rounded-full bg-gray-300"></div>}
                  </div>
                  <span className={`text-[10px] font-medium transition-colors ${selectedCategory === cat ? 'text-gray-900 font-bold' : 'text-gray-400'}`}>{cat}</span>
               </div>
            ))}
         </div>
         ...
         <div className="flex-1 overflow-y-auto p-5 pb-32 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            {displayProducts.map(p => {
               ...
               return (
                  <div key={p.id} className="flex gap-4 mb-6 group">
                     <div className="w-[96px] h-[96px] rounded-[20px] bg-gray-50 overflow-hidden shrink-0 relative shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-100">
                     ...
                     <div className="flex-1 flex flex-col justify-between py-1">
                        <div>
                           ...
                           <div className="flex items-center flex-wrap gap-1.5">
                              <span className="text-[10px] bg-gray-50 text-gray-500 px-2 py-0.5 rounded-md font-medium">{p.spec}</span>
                              {p.maxOrder < 999 && (
                                <span className="text-[10px] text-orange-600 px-2 py-0.5 rounded-md font-medium border border-orange-100 bg-orange-50/50">
                                   限 {p.maxOrder}
                                </span>
                              )}
                           </div>
                        ...
                        <div className="flex justify-between items-end">
                           <div className="flex items-baseline gap-0.5">
                              ...
                              <span className="text-[#00B42A] font-bold text-xl">{p.price.toFixed(2)}</span>
                           ...
                           {qty > 0 ? (
                             <div className="flex items-center gap-3 bg-gray-50 p-1 rounded-full border border-gray-100">
                                <button onClick={() => updateCart(p, -1)} className="w-7 h-7 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-600 active:scale-90 transition-transform">
                                   <Minus size={14} strokeWidth={3} />
                                </button>
                                <span className="text-sm font-bold w-4 text-center tabular-nums">{qty}</span>
                                <button onClick={() => updateCart(p, 1)} className="w-7 h-7 rounded-full bg-[#00B42A] flex items-center justify-center text-white active:scale-90 transition-transform shadow-md shadow-green-500/30">
      ...
      {cartCount > 0 && (
         <div className="absolute bottom-6 left-5 right-5 z-30 animate-in slide-in-from-bottom-4 duration-300">
             <div className="bg-[#1D2129]/95 backdrop-blur-xl text-white p-2 pl-6 rounded-full shadow-[0_20px_40px_rgba(0,0,0,0.2)] flex items-center justify-between border border-white/10 ring-1 ring-black/5">
                ...
                <button className="bg-[#00B42A] hover:bg-[#00a326] text-white px-8 py-3.5 rounded-full font-bold text-sm shadow-lg shadow-green-500/20 active:scale-95 transition-all">
                   去结算
                </button>
             </div>
         </div>
  ...
  const renderOrders = () => (
    <div className="flex flex-col h-full bg-[#F7F8FA]">
       {/* Header */}
       <div className="bg-white/80 backdrop-blur-md sticky top-0 z-20 border-b border-gray-50 transition-all">
          ...
          <div className="flex px-6 mt-2 relative">
             {['today', 'history'].map((tab) => (
                <button 
                  ...
                  className={`mr-8 py-3 text-sm font-bold transition-colors relative z-10 ${orderTimeTab === tab ? 'text-[#00B42A]' : 'text-gray-400'}`}
                >
                  {tab === 'today' ? '今日订单' : '历史订单'}
       ...
       <div className="flex-1 overflow-y-auto p-6 space-y-5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
          {MOCK_ORDERS.map((order, index) => {
             ...
             let statusConfig = { bg: "bg-green-100/50", text: "text-green-700", label: "已审核" };
             if (order.status === OrderStatus.Auditing) {
                statusConfig = { bg: "bg-blue-100/50", text: "text-blue-700", label: "审核中" };
             } else if (order.status === OrderStatus.Rejected) {
                statusConfig = { bg: "bg-red-100/50", text: "text-red-700", label: "已驳回" };
             }
             if (isModifiedDemo) {
                 statusConfig = { bg: "bg-blue-100/50", text: "text-blue-700", label: "审核中" };
             }
             ...
             return (
               <div key={order.id} className="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-white active:scale-[0.99] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.05)] group">
                  <div className="flex justify-between items-center mb-5">
                     <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm ${statusConfig.bg} ${statusConfig.text}`}>
                            {statusConfig.label === '已审核' ? <Check size={18} strokeWidth={3} /> : <Clock size={18} strokeWidth={3} />}
                        </div>
                        <div>
                  ...
                  {isModifiedDemo ? (
                     <div className="mb-5 bg-orange-50 rounded-2xl p-4 border border-orange-100 relative overflow-hidden">
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-orange-400"></div>
                        <div className="flex items-center gap-3 mb-2">
                           <span className="font-bold text-gray-900 text-sm">土豆 (15kg)</span>
                           <ArrowRight size={14} className="text-orange-400" />
                           <span className="font-bold text-orange-600 text-sm bg-white px-2 py-0.5 rounded shadow-sm">12kg</span>
                        </div>
                        <div className="flex items-start gap-1.5 text-xs text-orange-700/80">
                  ...
                  <div className="flex justify-end pt-2">
                     <button className="bg-gray-900 text-white text-xs font-bold px-6 py-3 rounded-xl shadow-lg shadow-gray-900/10 active:scale-95 transition-all hover:bg-black flex items-center gap-2 group-hover:pr-4">
  ...
  const renderProfile = () => (
    <div className="flex flex-col h-full bg-[#F7F8FA]">
       <div className="bg-white pt-20 pb-10 px-6 rounded-b-[40px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] relative z-10 mb-6">
          <div className="flex items-center gap-6">
             <div className="relative group cursor-pointer">
                 <div className="w-20 h-20 rounded-full p-1 border-2 border-gray-100 group-hover:border-[#00B42A] transition-colors">
          ...
          <div className="flex mt-10 divide-x divide-gray-100">
             ...
             <div className="flex-1 text-center">
                <div className="text-xl font-extrabold text-gray-900">96%</div>
